...
function call(id){
    ...
    socket.emit("admin_list_by_user", params, (messages) =>{
        ...
        messages.forEach(message => {
            ...
            if(message.admin_id === null){
                ...
                createDiv.innerHTML = `<span>Atendente:  ${message.text}`
                createDiv.innerHTML += `<span class="admin_date"> ${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>`
            }
...
function sendMessage(id){
    let text = document.getElementById(`send_message_${id}`).value;
    ...
    createDiv.className = "admin_message_admin"
    createDiv.innerHTML = `<span>Atendente:  ${params.text}`
    createDiv.innerHTML += `<span class="admin_date"> ${dayjs().format("DD/MM/YYYY HH:mm:ss")}</span>`

    divMessages.appendChild(createDiv) 

    text = document.getElementById(`send_message_${id}`).value = "";
}
